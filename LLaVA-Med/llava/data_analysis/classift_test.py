...
def main(dataset, model_name, probe, split, feature_type, n_epochs):
    data = [json.loads(line) for line in open(f"LLaVA-Med/data/eval/{dataset}_llava_val.jsonl")]
    random.seed(1234)
    random.shuffle(data)
    classes =  ['atelectasis', 'cardiomegaly', 'pleural effusion', 'infiltration', 'lung mass', 
